...
- (instancetype)init
{
  if (self = [super init]) {
    ...
    // myCentralManagerIdentifier removed as it caused NSException on init with newer react versions
    // propably needs implementation for "centralManager:(CBCentralManager *)central willRestoreState"
    // inoder to work properly
        ...
        if (targetCharacteristic) {
            ...
            if(withoutResponse){
                [self.bridge.eventDispatcher sendDeviceEventWithName:@"ble.write" body:@{
                                                    @"peripheralUuid": [self toNobleUuid:peripheralUuid],
                                                    @"serviceUuid": [self toNobleUuid:serviceUuid],
                                                    @"characteristicUuid": [self toNobleUuid:characteristicUuid]
                                                    }];
            }
